...
static int ovl_parse_opt(char *opt, struct ovl_config *config, bool remount)
{
	...
	/* Workdir is useless in non-upper or non-lower mount */
	if ((!config->upperdir || !config->lowerdir) && config->workdir) {
		pr_info("overlayfs: option \"workdir=%s\" is useless without upperdir and lowerdir, ignore\n",
			config->workdir);
		kfree(config->workdir);
		config->workdir = NULL;
	}
...
static int ovl_snapshot_remount(struct super_block *sb, int *flags, char *data)
{
	...
	/*
	 * Set config.snapshot to empty and parse remount options.
	 * If no new snapshot= option nor nosnapshot option was found,
	 * config.snapshot will remain empty and nothing will change.
...
static int ovl_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	if (!ufs->config.lowerdir && !ufs->config.upperdir) {
		if (!silent)
			pr_err("overlayfs: missing 'lowerdir' or 'upperdir'\n");
		goto out_free_config;
	}
	...
	if (ufs->config.upperdir) {
		if (!ufs->config.workdir && ufs->config.lowerdir) {
	...
	for (i = 0; i < numlower; i++) {
		...
		if ((ufs->upper_mnt &&
		     ufs->upper_mnt->mnt_sb != mnt->mnt_sb) ||
		    (i > 0 &&
		     ufs->lower_mnt[0]->mnt_sb != mnt->mnt_sb))
			ufs->config.redirect_fh = ufs->samefs = false;
	}
...
{
	register_as_snapshot();
	return register_filesystem(&ovl_fs_type);
}

static void __exit ovl_exit(void)
{
	unregister_as_snapshot();
