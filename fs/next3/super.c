...
static void next3_handle_error(struct super_block *sb)
{
	...
	if (!test_opt (sb, ERRORS_CONT)) {
		...
		NEXT3_SB(sb)->s_mount_opt |= NEXT3_MOUNT_ABORT;
		if (journal)
	...
	if (test_opt(sb, ERRORS_PANIC))
		panic("NEXT3-fs (device %s): panic forced after error\n",
			sb->s_id);
}
		 ...
		 const char * fmt, ...)
{
	...
	printk (KERN_CRIT "next3_abort called.\n");
		   ...
		   const char * fmt, ...)
{
	...
	va_start(args, fmt);
	printk(KERN_WARNING "NEXT3-fs warning (device %s): %s: ",
	       sb->s_id, function);
	vprintk(fmt, args);
	printk("\n");
	va_end(args);
 ...
 */
static struct block_device *next3_blkdev_get(dev_t dev)
{
...
fail:
	printk(KERN_ERR "NEXT3: failed to open journal device %s: %ld\n",
...
static inline void next3_show_quota_options(struct seq_file *seq, struct super_block *sb)
{
	...
	if (sbi->s_mount_opt & NEXT3_MOUNT_USRQUOTA)
		seq_puts(seq, ",usrquota");

	if (sbi->s_mount_opt & NEXT3_MOUNT_GRPQUOTA)
...
static struct dquot_operations next3_quota_operations = {
...
static struct quotactl_ops next3_qctl_operations = {
...
static next3_fsblk_t get_sb_block(void **data)
{
	...
	if (*options && *options != ',') {
		printk("NEXT3-fs: Invalid sb specification: %s\n",
		       (char *) *data);
		return 1;
	}
	if (*options == ',')
			  ...
			  next3_fsblk_t *n_blocks_count, int is_remount)
{
	...
	while ((p = strsep (&options, ",")) != NULL) {
		...
		case Opt_user_xattr:
		case Opt_nouser_xattr:
			printk("NEXT3 (no)user_xattr options not supported\n");
			break;
#endif
#ifdef CONFIG_NEXT3_FS_POSIX_ACL
		case Opt_acl:
		...
		case Opt_acl:
		case Opt_noacl:
			printk("NEXT3 (no)acl options not supported\n");
			break;
#endif
		...
		case Opt_journal_update:
			...
			/* Eventually we will want to be able to create
			   ...
			   journal file. */
			if (is_remount) {
				printk(KERN_ERR "NEXT3-fs: cannot specify "
				       "journal on remount\n");
				return 0;
			}
		...
		datacheck:
			if (is_remount) {
				if ((sbi->s_mount_opt & NEXT3_MOUNT_DATA_FLAGS)
					...
					break;
				printk(KERN_ERR
					"NEXT3-fs (device %s): Cannot change "
				...
				sbi->s_mount_opt &= ~NEXT3_MOUNT_DATA_FLAGS;
...
set_qf_name:
			if (sb_any_quota_loaded(sb) &&
				...
				printk(KERN_ERR
					"NEXT3-fs: Cannot change journaled "
			...
			if (!qname) {
				printk(KERN_ERR
					"NEXT3-fs: not enough memory for "
...
clear_qf_name:
			if (sb_any_quota_loaded(sb) &&
			    sbi->s_qf_names[qtype]) {
				printk(KERN_ERR "NEXT3-fs: Cannot change "
...
set_qf_format:
		...
		case Opt_noquota:
			if (sb_any_quota_loaded(sb)) {
				printk(KERN_ERR "NEXT3-fs: Cannot change quota "
		...
		case Opt_grpquota:
			printk(KERN_ERR
				"NEXT3-fs: quota options not supported.\n");
			break;
		case Opt_usrjquota:
		case Opt_grpjquota:
		case Opt_offusrjquota:
		...
		case Opt_jqfmt_vfsv0:
			printk(KERN_ERR
				"NEXT3-fs: journaled quota options not "
				"supported.\n");
			break;
		case Opt_noquota:
			break;
		...
		case Opt_resize:
			if (!is_remount) {
				printk("NEXT3-fs: resize option only available "
					"for remount\n");
				return 0;
			}
		...
		default:
			printk (KERN_ERR
				"NEXT3-fs: Unrecognized mount option \"%s\" "
				"or missing value\n", p);
			return 0;
		}
	}
	...
	if (sbi->s_qf_names[USRQUOTA] || sbi->s_qf_names[GRPQUOTA]) {
		...
		if ((sbi->s_qf_names[USRQUOTA] &&
				(sbi->s_mount_opt & NEXT3_MOUNT_GRPQUOTA)) ||
		    (sbi->s_qf_names[GRPQUOTA] &&
				(sbi->s_mount_opt & NEXT3_MOUNT_USRQUOTA))) {
			printk(KERN_ERR "NEXT3-fs: old and new quota "
					"format mixing.\n");
			return 0;
		}

		if (!sbi->s_jquota_fmt) {
			printk(KERN_ERR "NEXT3-fs: journaled quota format "
					"not specified.\n");
			return 0;
		}
	} else {
			    ...
			    int read_only)
{
	...
	if (le32_to_cpu(es->s_rev_level) > NEXT3_MAX_SUPP_REV) {
		printk (KERN_ERR "NEXT3-fs warning: revision level too high, "
	...
	if (!(sbi->s_mount_state & NEXT3_VALID_FS))
		printk (KERN_WARNING "NEXT3-fs warning: mounting unchecked fs, "
			"running e2fsck is recommended\n");
	else if ((sbi->s_mount_state & NEXT3_ERROR_FS))
		printk (KERN_WARNING
			"NEXT3-fs warning: mounting fs with errors, "
			"running e2fsck is recommended\n");
	else if ((__s16) le16_to_cpu(es->s_max_mnt_count) >= 0 &&
		 le16_to_cpu(es->s_mnt_count) >=
		 (unsigned short) (__s16) le16_to_cpu(es->s_max_mnt_count))
		printk (KERN_WARNING
			"NEXT3-fs warning: maximal mount count reached, "
			"running e2fsck is recommended\n");
	else if (le32_to_cpu(es->s_checkinterval) &&
		(le32_to_cpu(es->s_lastcheck) +
			le32_to_cpu(es->s_checkinterval) <= get_seconds()))
		printk (KERN_WARNING
			"NEXT3-fs warning: checktime reached, "
			"running e2fsck is recommended\n");
#if 0
	...
	next3_commit_super(sb, es, 1);
	if (test_opt(sb, DEBUG))
		printk(KERN_INFO "[NEXT3 FS bs=%lu, gc=%lu, "
	...
	printk(KERN_INFO "NEXT3 FS on %s, ", sb->s_id);
				 ...
				 struct next3_super_block * es)
{
	...
	if (bdev_read_only(sb->s_bdev)) {
		printk(KERN_ERR "NEXT3-fs: write access "
	...
	if (s_flags & MS_RDONLY) {
		printk(KERN_INFO "NEXT3-fs: %s: orphan cleanup on readonly fs\n",
		       sb->s_id);
		sb->s_flags &= ~MS_RDONLY;
	}
#ifdef CONFIG_QUOTA
	...
	for (i = 0; i < MAXQUOTAS; i++) {
		if (NEXT3_SB(sb)->s_qf_names[i]) {
			...
			if (ret < 0)
				printk(KERN_ERR
					"NEXT3-fs: Cannot turn on journaled "
					"quota: error %d\n", ret);
		}
	}
#endif
...
#define PLURAL(x) (x), ((x)==1) ? "" : "s"
	...
	if (nr_orphans)
		printk(KERN_INFO "NEXT3-fs: %s: %d orphan inode%s deleted\n",
		       sb->s_id, PLURAL(nr_orphans));
	if (nr_truncates)
		printk(KERN_INFO "NEXT3-fs: %s: %d truncate%s cleaned up\n",
...
static int next3_fill_super (struct super_block *sb, void *data, int silent)
{
	...
	next3_fsblk_t block;
	next3_fsblk_t sb_block = get_sb_block(&data);
	next3_fsblk_t logic_sb_block;
	...
	if (!blocksize) {
		printk(KERN_ERR "NEXT3-fs: unable to set blocksize\n");
		goto out_fail;
	}
	...
	if (!(bh = sb_bread(sb, logic_sb_block))) {
		printk (KERN_ERR "NEXT3-fs: unable to read superblock\n");
		goto out_fail;
	}
	/*
	...
	if (le32_to_cpu(es->s_rev_level) == NEXT3_GOOD_OLD_REV &&
		...
		printk(KERN_WARNING
		       "NEXT3-fs warning: feature flags set on rev 0 fs, "
		       "running e2fsck is recommended\n");
	/*
	...
	if (!(sb->s_flags & MS_RDONLY) && features) {
		printk(KERN_ERR "NEXT3-fs: %s: couldn't mount RDWR because of "
		       "unsupported optional features (%x).\n",
		       sb->s_id, le32_to_cpu(features));
		goto failed_mount;
	}
	...
	if (sb->s_blocksize != blocksize) {
		 ...
		 */
		if (blocksize < hblock) {
			printk(KERN_ERR "NEXT3-fs: blocksize %d too small for "
			       "device blocksize %d.\n", blocksize, hblock);
			goto failed_mount;
		}
		...
		if (!sb_set_blocksize(sb, blocksize)) {
			printk(KERN_ERR "NEXT3-fs: bad blocksize %d.\n",
				blocksize);
			goto out_fail;
		}
		logic_sb_block = (sb_block * NEXT3_MIN_BLOCK_SIZE) / blocksize;
		...
		if (!bh) {
			printk(KERN_ERR
			       "NEXT3-fs: Can't read superblock on 2nd try.\n");
			goto failed_mount;
		}
		es = (struct next3_super_block *)(((char *)bh->b_data) + offset);
		sbi->s_es = es;
		if (es->s_magic != cpu_to_le16(NEXT3_SUPER_MAGIC)) {
			printk (KERN_ERR
				"NEXT3-fs: Magic mismatch, very weird !\n");
			goto failed_mount;
		}
	}
	...
	if (le32_to_cpu(es->s_rev_level) == NEXT3_GOOD_OLD_REV) {
		...
		if ((sbi->s_inode_size < NEXT3_GOOD_OLD_INODE_SIZE) ||
			...
			printk (KERN_ERR
				"NEXT3-fs: unsupported inode size: %d\n",
				sbi->s_inode_size);
			goto failed_mount;
		}
	}
	...
	if (blocksize != sbi->s_frag_size) {
		printk(KERN_ERR
		       "NEXT3-fs: fragsize %lu != blocksize %u (unsupported)\n",
	...
	if (sbi->s_blocks_per_group > blocksize * 8) {
		printk (KERN_ERR
			"NEXT3-fs: #blocks per group too big: %lu\n",
			sbi->s_blocks_per_group);
		goto failed_mount;
	}
	if (sbi->s_frags_per_group > blocksize * 8) {
		printk (KERN_ERR
			"NEXT3-fs: #fragments per group too big: %lu\n",
			sbi->s_frags_per_group);
		goto failed_mount;
	}
	if (sbi->s_inodes_per_group > blocksize * 8) {
		printk (KERN_ERR
			"NEXT3-fs: #inodes per group too big: %lu\n",
	...
	if (le32_to_cpu(es->s_blocks_count) >
		...
		if (sizeof(sector_t) < 8)
			printk(KERN_WARNING "NEXT3-fs: CONFIG_LBDAF not "
					"enabled\n");
		goto failed_mount;
	}
	...
	if (sbi->s_group_desc == NULL) {
		printk (KERN_ERR "NEXT3-fs: not enough memory\n");
		goto failed_mount;
	}
	...
	if (test_opt(sb, NOBH)) {
		if (!(test_opt(sb, DATA_FLAGS) == NEXT3_MOUNT_WRITEBACK_DATA)) {
			printk(KERN_WARNING "NEXT3-fs: Ignoring nobh option - "
	...
	printk (KERN_INFO "NEXT3-fs: mounted filesystem with %s data mode.\n",
...
cantfind_next3:
	if (!silent)
		printk(KERN_ERR "VFS: Can't find next3 filesystem on dev %s.\n",
		       sb->s_id);
	goto failed_mount;
				   ...
				   unsigned int journal_inum)
{
	...
	if (IS_ERR(journal_inode)) {
		printk(KERN_ERR "NEXT3-fs: no journal found.\n");
		return NULL;
	}
	if (!journal_inode->i_nlink) {
		make_bad_inode(journal_inode);
		iput(journal_inode);
		printk(KERN_ERR "NEXT3-fs: journal inode is deleted.\n");
		return NULL;
	}
	...
	if (!S_ISREG(journal_inode->i_mode)) {
		printk(KERN_ERR "NEXT3-fs: invalid journal inode.\n");
		iput(journal_inode);
		return NULL;
	}
				       ...
				       dev_t j_dev)
{
	...
	bdev = next3_blkdev_get(j_dev);
	if (bdev == NULL)
		return NULL;

	if (bd_claim(bdev, sb)) {
		printk(KERN_ERR
		        "NEXT3: failed to claim external journal device.\n");
	...
	if (blocksize < hblock) {
		printk(KERN_ERR
			"NEXT3-fs: blocksize too small for journal device.\n");
		goto out_bdev;
	}
	...
	if (!(bh = __bread(bdev, sb_block, blocksize))) {
		printk(KERN_ERR "NEXT3-fs: couldn't read superblock of "
		       "external journal\n");
		goto out_bdev;
	}
	...
	if ((le16_to_cpu(es->s_magic) != NEXT3_SUPER_MAGIC) ||
		...
		printk(KERN_ERR "NEXT3-fs: external journal has "
					"bad superblock\n");
		brelse(bh);
		goto out_bdev;
	}
	...
	if (memcmp(NEXT3_SB(sb)->s_es->s_journal_uuid, es->s_uuid, 16)) {
		printk(KERN_ERR "NEXT3-fs: journal UUID does not match\n");
		brelse(bh);
		goto out_bdev;
	}
	...
	journal = journal_init_dev(bdev, sb->s_bdev,
					start, len, blocksize);
	if (!journal) {
		printk(KERN_ERR "NEXT3-fs: failed to create device journal\n");
		goto out_bdev;
	}
	journal->j_private = sb;
	...
	if (!buffer_uptodate(journal->j_sb_buffer)) {
		printk(KERN_ERR "NEXT3-fs: I/O error on journal device\n");
		goto out_journal;
	}
	if (be32_to_cpu(journal->j_superblock->s_nr_users) != 1) {
		printk(KERN_ERR "NEXT3-fs: External journal has more than one "
					"user (unsupported) - %d\n",
			     ...
			     unsigned long journal_devnum)
{
	...
	if (journal_devnum &&
		...
		printk(KERN_INFO "NEXT3-fs: external journal device major/minor "
	...
	if (NEXT3_HAS_INCOMPAT_FEATURE(sb, NEXT3_FEATURE_INCOMPAT_RECOVER)) {
		if (sb->s_flags & MS_RDONLY) {
			printk(KERN_INFO "NEXT3-fs: INFO: recovery "
			...
			printk (KERN_INFO "NEXT3-fs: write access will "
					"be enabled during recovery.\n");
		}
	}

	if (journal_inum && journal_dev) {
		printk(KERN_ERR "NEXT3-fs: filesystem has both journal "
		       "and inode journals!\n");
		return -EINVAL;
	}
	...
	if (!really_read_only && test_opt(sb, UPDATE_JOURNAL)) {
		...
		if (err)  {
			printk(KERN_ERR "NEXT3-fs: error updating journal.\n");
			journal_destroy(journal);
			return err;
		}
	}
	...
	if (err) {
		printk(KERN_ERR "NEXT3-fs: error loading journal.\n");
		journal_destroy(journal);
		return err;
	}
			       ...
			       unsigned int journal_inum)
{
	...
	if (sb->s_flags & MS_RDONLY) {
		printk(KERN_ERR "NEXT3-fs: readonly filesystem when trying to "
				"create journal.\n");
		return -EROFS;
	}
	...
	printk(KERN_INFO "NEXT3-fs: creating new journal on inode %u\n",
	       journal_inum);

	err = journal_create(journal);
	if (err) {
		printk(KERN_ERR "NEXT3-fs: error creating journal.\n");
		journal_destroy(journal);
		return -EIO;
	}
...
static int next3_remount (struct super_block * sb, int * flags, char * data)
{
	...
	if (sbi->s_mount_opt & NEXT3_MOUNT_ABORT)
	...
	if ((*flags & MS_RDONLY) != (sb->s_flags & MS_RDONLY) ||
		...
		if (*flags & MS_RDONLY) {
			...
			if ((ret = NEXT3_HAS_RO_COMPAT_FEATURE(sb,
				...
				printk(KERN_WARNING "NEXT3-fs: %s: couldn't "
...
static int next3_quota_on_mount(struct super_block *sb, int type)
{
	return vfs_quota_on_mount(sb, NEXT3_SB(sb)->s_qf_names[type],
			 ...
			 char *name, int remount)
{
	...
	if (NEXT3_SB(sb)->s_qf_names[type]) {
		...
		if (path.dentry->d_parent != sb->s_root)
			printk(KERN_WARNING
				"NEXT3-fs: Quota file not on filesystem root. "
				...
				const char *data, size_t len, loff_t off)
{
	...
	if (!handle) {
		printk(KERN_WARNING "NEXT3-fs: Quota write (off=%Lu, len=%Lu)"
