...
/* Flags that should be inherited by new inodes from their parent. */
#define NEXT3_FL_INHERITED (NEXT3_SECRM_FL | NEXT3_UNRM_FL | NEXT3_COMPR_FL |\
			   NEXT3_SYNC_FL | NEXT3_NODUMP_FL |\
			   NEXT3_NOATIME_FL | NEXT3_COMPRBLK_FL |\
			   NEXT3_NOCOMPR_FL | NEXT3_JOURNAL_DATA_FL |\
			   NEXT3_NOTAIL_FL | NEXT3_DIRSYNC_FL)
#endif
...
/* NEXT3_MOUNT_OLDALLOC was there */
...
#define next3_set_bit			__set_bit_le
#define next3_set_bit_atomic		ext2_set_bit_atomic
#define next3_clear_bit			__clear_bit_le
#define next3_clear_bit_atomic		ext2_clear_bit_atomic
#define next3_test_bit			test_bit_le
#define next3_find_next_zero_bit		find_next_zero_bit_le
 ...
 */
enum {
	NEXT3_STATE_JDATA,		/* journaled data exists */
	NEXT3_STATE_NEW,			/* inode is newly created */
	NEXT3_STATE_XATTR,		/* has in-inode xattrs */
	NEXT3_STATE_FLUSH_ON_CLOSE,	/* flush dirty pages on close */
};

static inline int next3_test_inode_state(struct inode *inode, int bit)
{
	return test_bit(bit, &NEXT3_I(inode)->i_state_flags);
}

static inline void next3_set_inode_state(struct inode *inode, int bit)
{
	set_bit(bit, &NEXT3_I(inode)->i_state_flags);
}

static inline void next3_clear_inode_state(struct inode *inode, int bit)
{
	clear_bit(bit, &NEXT3_I(inode)->i_state_flags);
}
#else
...
extern int next3_trim_fs(struct super_block *sb, struct fstrim_range *range);
...
/* fsync.c */
extern int next3_sync_file(struct file *, loff_t, loff_t, int);
...
/* ialloc.c */
extern struct inode * next3_new_inode (handle_t *, struct inode *,
...
extern void next3_truncate(struct inode *inode);
...
extern long next3_fallocate(struct file *file, int mode,
...
/* super.c */
extern __printf(3, 4)
void next3_error(struct super_block *, const char *, const char *, ...);
extern void __next3_std_error (struct super_block *, const char *, int);
extern __printf(3, 4)
void next3_abort(struct super_block *, const char *, const char *, ...);
extern __printf(3, 4)
void next3_warning(struct super_block *, const char *, const char *, ...);
extern __printf(3, 4)
void next3_msg(struct super_block *, const char *, const char *, ...);
