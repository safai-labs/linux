...
int next3_get_blocks_handle(handle_t *handle, struct inode *inode,
		...
		int create)
{
#warning 500 LoC function.  Deep nesting.  i tried my best. Must break/simplify.
#warning cant review this fxn until it gets cleaned up and broken up
	int err = -EIO;
	int offsets[4];
	Indirect chain[4];
	Indirect *partial;
	...
	while (count < maxblocks && count <= blocks_to_boundary) {
		...
		if (blk) {
			...
			if (!sbh && copies == 1 &&
				 ...
				 * cache it in bh_result for subsequent
				 * calls */
	...
	if (shrink && partial == chain + depth - 1) {
		...
		if (freed_blocks == mapped_blocks &&
			...
			for (p = (__le32 *)(partial->bh->b_data);
			     p < partial->p; p++)
#warning you can fold this "*p" part into the for loop itself
				if (*p)
					break;
		}
		if (p == partial->p)
			...
			struct buffer_head *bh_result, int create)
{
	...
	if (create && !handle) {	/* Direct IO write... */
		...
		handle = next3_journal_start(inode, DIO_CREDITS +
				2 * NEXT3_QUOTA_TRANS_BLOCKS(inode->i_sb));
		if (IS_ERR(handle)) {
			ret = PTR_ERR(handle);
			goto out;
		}
				...
				struct page *page, void *fsdata)
{
	...
	if (pos + len > inode->i_size && next3_can_truncate(inode))
		next3_orphan_add(handle, inode);
	NEXT3_I(inode)->i_state |= NEXT3_STATE_JDATA;
...
static sector_t next3_bmap(struct address_space *mapping, sector_t block)
{
	...
	if (NEXT3_I(inode)->i_state & NEXT3_STATE_JDATA) {
		/*
...
void next3_truncate(struct inode *inode)
{
	...
	if (inode->i_size == 0 && next3_should_writeback_data(inode))
		ei->i_state |= NEXT3_STATE_FLUSH_ON_CLOSE;
 ...
 */
int next3_write_inode(struct inode *inode, int wait)
{
...
int next3_setattr(struct dentry *dentry, struct iattr *attr)
{
	...
	if ((ia_valid & ATTR_UID && attr->ia_uid != inode->i_uid) ||
		...
		handle = next3_journal_start(inode, 2*(NEXT3_QUOTA_INIT_BLOCKS(inode->i_sb)+
					NEXT3_QUOTA_DEL_BLOCKS(inode->i_sb))+3);
		if (IS_ERR(handle)) {
...
static int next3_writepage_trans_blocks(struct inode *inode)
{
	...
	ret += 2*NEXT3_QUOTA_TRANS_BLOCKS(inode->i_sb);
#endif
