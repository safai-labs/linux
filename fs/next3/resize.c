				  ...
				  struct next3_new_group_data *input)
{
	...
	for (i = 0, bit = 1, block = start + 1;
		...
		unlock_buffer(gdb);
		err = next3_journal_dirty_metadata(handle, gdb);
		if (err) {
			brelse(gdb);
			goto exit_bh;
		}
	...
	for (i = 0, block = input->inode_table, bit = block - start;
		...
		err = next3_journal_dirty_metadata(handle, it);
		if (err) {
			brelse(it);
			goto exit_bh;
		}
		       ...
		       struct buffer_head **primary)
{
	...
	*primary = sb_bread(sb, gdblock);
	...
	err = next3_journal_dirty_metadata(handle, dind);
	if (err)
	...
	err = next3_journal_dirty_metadata(handle, *primary);
	if (err)
		goto exit_group_desc;
		      ...
		      next3_fsblk_t n_blocks_count)
{
	...
	if (test_opt(sb, DEBUG))
		printk(KERN_DEBUG "NEXT3-fs: extending last group from "E3FSBLK
		       " upto "E3FSBLK" blocks\n",
	...
	mutex_lock(&NEXT3_SB(sb)->s_resize_lock);
	if (o_blocks_count != le32_to_cpu(es->s_blocks_count)) {
		next3_warning(sb, __func__,
			     "multiple resizers run on filesystem!");
		mutex_unlock(&NEXT3_SB(sb)->s_resize_lock);
	...
	next3_debug("freeing blocks "E3FSBLK" through "E3FSBLK"\n",
