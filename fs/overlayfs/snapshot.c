					...
					struct vfsmount *snapmnt)
{
	...
	/*
	 * maybe an uninitialized snapdentry after snapshot take
	 * or after parent copy up, but it could also mean that
	 * dentry has an ancestor with a negative (whiteout) snapdentry
	 * meaning that dentry is inside a new directory and should not
	 * be copied to snapshot.
	 */
	if (!snapdentry)
		return -ENOENT;

	/*
	 * snapdentry is not NULL.
	 * NULL snapmnt or snapdentry->d_sb != snapmnt->mnt_sb
	 * implies stale snapdentry from an older snapshot
	 */
	if (unlikely(!snapmnt ||
